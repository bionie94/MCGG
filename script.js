...
function generateDropdowns() {
  ...
  ["Round I-2", "Round I-3", "Round I-4"].forEach((title, idx) => {
    ...
    if (idx === 0) {
      // Round I-2: first match locked to players[0], then 3 matches
      section.appendChild(makeMatch(players[0], true));
      for (let i = 0; i < 3; i++) section.appendChild(makeMatch(null, false));
    } else {
      // Round I-3 and I-4: four selectable matches each
...
function makeMatch(defaultPlayer, locked) {
  ...
  function createInput(value, disabled = false) {
    if (disabled) {
      ...
      // populate with players (exclude players[0] from left-side if desired? keep full list so selection possible)
      (players || []).forEach((p) => {
  ...
  if (defaultPlayer && typeof defaultPlayer === "string" && defaultPlayer.length > 0) {
    let inputFixed = createInput(defaultPlayer, true);
...
function getRound(roundIdx) {
  ...
  if (!section) {
    alert("Round not found. Please Generate Rounds first.");
    return null;
  }
...
function solve() {
  // Read rounds (I-2, I-3, I-4) and validate
  let rounds = [];
  for (let i = 0; i < 3; i++) {
    let r = getRound(i);
    if (!r) return;
