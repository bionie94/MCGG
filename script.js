...
function generateDropdowns() {
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    if (!el) {
      alert("Input player tidak ditemui (P" + i + ").");
      return;
    }
  ...
  if (!roundsDiv) {
    alert("Element #rounds tidak ditemui dalam HTML.");
    return;
  }
  roundsDiv.innerHTML = "";
...
function getRound(roundIdx) {
  let roundsParent = document.getElementById("rounds");
  if (!roundsParent) {
    alert("Sila klik Generate Rounds dahulu.");
    return null;
  }
  let section = roundsParent.children[roundIdx];
  if (!section) {
    alert("Round index tidak wujud.");
    return null;
  }
